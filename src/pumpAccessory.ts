...
export class PumpAccessory {
	...
	constructor(
		...
		private readonly _numSpeedSettings: number
	) {
		...
		this.numSpeedSettings = _numSpeedSettings;
		this.name = (pumpNumber == 0 ? "Circulation Pump" : "Pump " + (pumpNumber - 1));
	...
	setOn(value: CharacteristicValue, callback: CharacteristicSetCallback) {
		...
		//Don't issue the command right away.  See comment on method below
	...
	setRotationSpeed(value: CharacteristicValue, callback: CharacteristicSetCallback) {
		...
		//Do not issue command right away.  See comment on method below
		this.scheduleSetSpeed(speed);
	...
	getRotationSpeed(callback: CharacteristicSetCallback) {
		if (!this.platform.isCurrentlyConnected()) {
			...
			this.platform.log.debug('Get', this.name, 'Speed <-', value, '(numSpeedSettings:', this.numSpeedSettings, '|speed:', speed, ')which is', SpaClient.getSpeedAsString(this.numSpeedSettings, speed), this.platform.status());
			callback(null, value);
		}
	}
	...
	updateCharacteristics() {
		...
		//this.platform.log.debug(this.name,'updating to',isOn ? 'On' : 'Off','and',speed,'which is', SpaClient.getSpeedAsString(this.numSpeedSettings, speed));
	...
	private setSpeed(speed: number) {
		this.platform.log.debug(this.name, 'actually setting speed to', speed, 'which is', SpaClient.getSpeedAsString(this.numSpeedSettings, speed), this.platform.status());
